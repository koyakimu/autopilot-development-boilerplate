...
- 既存テストを壊していない
- **Spec チェック（Stop フック）の最新結果が OK** — フックが毎ターン自動で走り、Spec・Design との一致を確認する。評価器はフックが会話に出した結果を判定材料にする
- 非機能要件のうち委譲したもの（`/security-review` 等）の実行結果が達成条件に含まれる
...
- 自前のループ実装 — `/goal` が担当
- peer-review の起動 — Spec チェック（Stop フック）が Build 中に自動で走るため、skill 側でレビュー起動はしない
