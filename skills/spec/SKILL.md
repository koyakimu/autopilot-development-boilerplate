...
Design文書の **What** セクションに記載された全機能を確認し、ユーザーと対話して**初回サイクルのMVPスコープ**を決定する:

1. 全機能を一覧化し、**MVP（初回サイクルで実装）** と **Future（後続サイクルで実装）** に分類する提案を行う
2. ユーザーの判断を仰ぎ、MVPスコープを確定する
3. **Futureに分類された機能は `docs/apd/todo.md` にToDoとして記録する**（起源: `Phase 1 Spec生成中（MVPスコーピング）`、経緯: MVPから除外した理由）
4. 確定したMVPスコープの機能についてのみSpecを生成する

### 生成ルール

1. MVPスコープに含まれる機能について Spec を作成する
