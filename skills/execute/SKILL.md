...
> ⛔ Contractが未承認です。`/apd:contract` でContract生成とAI Checkpointの承認を完了してから、再度 `/apd:execute` を実行してください。
>
...
5. **SpecのGiven/When/Then形式の受け入れ条件（AC）から直接テストコードを生成する**（ACがそのままテストケースになる）
6. テストを書いてから実装する（TDD推奨だが必須ではない）
7. 全テストがパスすることを確認する
