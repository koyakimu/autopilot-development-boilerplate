...
`docs/apd/todo.md` に `status: open` の項目がある場合は、一覧を提示して「これらのToDoに取り組みますか？」と確認する。ユーザーがToDoから選択した場合はその内容を変更内容として扱う。
...
- `docs/apd/decisions/*.md` — 既存Decision Recordsの有無
- `docs/apd/todo.md` — 未着手のToDo一覧（`status: open` の項目を確認）
...
| トリガー | 判定基準 | 通過フェーズ |
|---------|---------|------------|
| `new_product` | 新プロダクト or Design文書の枠を超える方向転換 | Phase 0 → 1 → 2 |
| `feature_addition` | 既存Design内の新機能追加 | Phase 1 → 2 |
| `bug_fix` | バグ修正 or 小さな改善 | Phase 1(Amendment) → 2 |
| `tech_change` | リファクタ・依存更新・技術的改善 | Decision Record → 2 |
...
| `tech_change` | Decision Recordを作成し、`/apd:build` を実行して実装を開始 |
