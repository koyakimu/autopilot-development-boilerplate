...
Phase 2は「AIの時間」である。AIが自律でContractを生成し、AIチェックポイントで品質を検証した後、自動承認する（エスカレーション項目がある場合のみ人間に確認）。
...
- **verdict: approve, escalation_required: false** → 自動承認処理を実行し、Phase 3へ進む
- **verdict: approve, escalation_required: true** → エスカレーション項目のみユーザーに提示し、判断を得てから承認処理を実行
- **verdict: request_changes** → 指摘事項を修正してContractを更新し、再度チェックポイントを実行

### 自動承認処理

AI Checkpointが approve を返し、エスカレーション不要の場合、以下を **全て自動実行する**:
...
3. 承認結果のサマリーを報告し、「`/apd:execute` を実行してPhase 3に進んでください」と案内する
