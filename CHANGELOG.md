...
### Added
- `scripts/migrate-to-1.0.sh` — 既存 APD プロジェクト (0.x) を新 flat 構造に変換するスクリプト。バックアップ → 移動・リネーム → レポート出力。`--dry-run` 対応。git mv で履歴保持。冪等性チェックあり (re-run で no-op)
- `MIGRATION.md` — 旧→新マイグレーションガイド。スクリプトの使い方、自動化される範囲、手動レビュー必要箇所 (frontmatter フィールド名、本文中のパス参照、CLAUDE.md 等)、ロールバック手順、トラブルシューティング
