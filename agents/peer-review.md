...
別コンテキストの実装をレビューし、仕様との整合性、テスト品質を検証します。
...
#### D. 対立的検証（Adversarial Testing）
- **積極的に壊しにいく視点**でレビューする
- 実装が想定していない入力・状態遷移・競合条件を探索する
- 「このコードが壊れるとしたらどんな場合か？」を常に問う
- 発見した障害ケースに対応するテストが存在しない場合はfindingとして報告する

### 3. 重要度分類

各findingに重要度を付与する:

- **blocker** — 必ず修正が必要。Spec違反、テスト欠落
...
### [BLOCKER] {問題の具体的な説明}
- **Category**: spec_compliance / cross_context / test_quality
- **Location**: {ファイルパス:行番号}
- **Suggestion**: {修正提案}
