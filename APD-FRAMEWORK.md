...
- **Phase 2〜3（Contract〜Execute）= AIの時間**: AI自律実行。人間はエスカレーション対応と完成品確認のみ
...
- **下流（Phase 2-3）**: 完成品を確認する — 動く成果物が意図通りかを確認する
...
graph LR
    subgraph "人間の時間"
        ...
        B -->|HC1: 仕様を決める| C[Phase 2: Contract]
    end
    subgraph "AIの時間"
        C -->|AI CP → 自動承認| D[Phase 3: Execute]
        D -->|Peer Review + AI CP| E[HC3: 完成品を確認する]
    end
```

### MVPファーストな開発フロー

新規プロダクト開発では、Design文書で完成形のビジョンを描きつつ、最初のサイクルではMVPに絞って実装する:

```mermaid
graph TD
    A[Design: 完成形のビジョン] --> B[Spec: MVPスコーピング]
    B -->|MVP機能| C[Contract → Execute]
    B -->|Future機能| D[ToDo に記録]
    C --> E[完成品確認]
    D -->|次のサイクル| F["/apd:cycle で取り組む"]
```

- Design文書は完成形を描く（北極星）
- Specフェーズで全機能をMVP / Futureに分類
- Future機能はToDoとして記録し、次のサイクルで取り組む
...
graph TD
    ...
    P2["Phase 2: Contract<br/>AI自律"] -->|"AI Checkpoint<br/>自動承認"| P3
    P3["Phase 3: Execute<br/>AI自律・並列実行"] -->|"Peer Review + AI Checkpoint"| HC3
    HC3["Human Checkpoint 3<br/>完成品確認"]

    style P0 fill:#e1f5fe
    style P1 fill:#e1f5fe
    style P2 fill:#fff3e0
    style P3 fill:#fff3e0
    style HC3 fill:#e8f5e9
```
...
新規プロダクト開発時は、Design文書の全機能からMVP / Futureを分類する:

```mermaid
graph LR
    A["Design: What セクションの全機能"] --> B{MVPスコーピング}
    B -->|MVP| C["Spec生成"]
    B -->|Future| D["docs/apd/todo.md に記録"]
```

1. 全機能を一覧化し、MVP / Future に分類する提案を行う
2. ユーザーの判断を仰ぎ、MVPスコープを確定する
3. Future機能は `docs/apd/todo.md` にToDoとして記録する
4. MVPスコープの機能についてのみSpecを生成する
...
AI自律で生成する。AI Checkpointで品質を検証した後、自動承認する（エスカレーション項目がある場合のみ人間に確認）。

```mermaid
graph LR
    A[Contract生成] --> B[AI Checkpoint]
    B -->|approve + エスカレーションなし| C[自動承認 → Phase 3へ]
    B -->|approve + エスカレーションあり| D[人間に確認 → 承認]
    B -->|request_changes| E[修正 → 再チェック]
    D --> C
    E --> B
```
...
- **どのフェーズからでも追記OK**: Design対話中でもExecute中でも、思いついた時に記録
...
graph TD
    ...
    B["Phase 1: Spec生成中<br/>MVPから除外した機能"] -->|追記| T
    C["Phase 3: Execute中<br/>発見した改善点"] -->|追記| T
...
## T-001: {タイトル}
- **status**: open / done
- **起源**: Phase 0 Design対話中 / Phase 1 Spec生成中 / Phase 3 Execute中
...
graph TD
    ...
    T -->|新プロダクト / 大きな方向転換| F["Design → Spec → Contract → Execute<br/>（フルサイクル）"]
    T -->|新機能追加| A["Spec → Contract差分 → Execute"]
    T -->|バグ修正 / 小さな改善| B["Spec Amendment → Execute"]
    T -->|技術的変更| C["Contract Amendment → Execute"]
